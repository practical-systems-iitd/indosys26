...
# STATE held by the Reducer
  ...
  server_socket: socket.socket         # Socket on which it will listen from the mapper
...
2. Reducers follow a similar structure as Mappers. The process simply starts
some threads and then waits for them to complete. Only the command handler
thread modifies the state to avoid races. Other threads just put commands in the
_command queue_.
...
channel upon receiving a checkpoint marker until it receives checkpoint markers
from all the channels. The state contains a barrier for this purpose.

Another difference from mappers is that reducers never send a done message to
the coordinator as it does not really know whether it is done (it might receive
yet more messages from the mappers).
...
a view of all the workers, i.e what the coordinator knows about the state of
each worker.
...
self.id: Final[str]                 # It can be Mapper_0, Mapper_1, Reducer_0, Reducer_1.
self.is_mapper: Final[bool]         # True, if a worker is mapper. False for reducers.
self.last_hb_recvd: int             # When did the coordinator receive last heartbeat (in seconds) from this worker.
...
messages.
   ...
   1. Sender thread reads the current phase of the system through phase queue.
      ...
      phase, it sends appropriate messages to the workers. 

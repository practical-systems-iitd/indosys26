...
from constants import FNAME, HEARTBEAT_INTERVAL, MAX_CKPT_ID, STREAMS, COORDINATOR_PORT
...
TYPE_RECOVER_MARKER: Final[int] = 2
  ...
  def recover(self, recovery_id: int, checkpoint_id: int):
    ...
    if checkpoint_id == -1:
      ...
      with open(filename, 'r') as file:
        ...
        self.last_stream_id = l.encode()

    # Re-create sockets
    self.is_wc_done = False
    for i, rs in enumerate(self.reducer_sockets):
      rs.close()
      self.reducer_sockets[i] = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
  ...
  def handle(self, state: MapperState):
    ...
    for idx, r_sock in enumerate(state.reducer_sockets):
      ...
      msg_bytes = Message(msg_type=MT.FWD_CHECKPOINT, source=state.id, mapper_idx=state.idx,
                          checkpoint_id=self.checkpoint_id, recovery_id=state.last_recovery_id).serialize()
    ...
    if self.checkpoint_id == MAX_CKPT_ID:
      chkack_msg = Message(msg_type=MT.LAST_CHECKPOINT_ACK, source=state.id, checkpoint_id=self.checkpoint_id)
    else:
...
class Recover(Cmd):
  ...
  def handle(self, state: MapperState):
    logging.info(f"{state.id} recovering from checkpoint {self.checkpoint_id}")

...
Any violation means the checkpoint/recovery protocol produced an
inconsistent (non-Chandy-Lamport) snapshot: a lost or double-counted word.
"""
import glob
import json
import re
...
from constants import MAX_CKPT_ID, NUM_MAPPERS, NUM_REDUCERS
...
def check() -> bool:
    ...
    ordinary_ids = sorted(k for k in by_ckpt if k != MAX_CKPT_ID)
    ordered_ids = ordinary_ids + ([MAX_CKPT_ID] if MAX_CKPT_ID in by_ckpt else [])
    ...
    for ckpt_id in ordered_ids:
        ...
        if a < prev_total:
            print(f"[ckpt {ckpt_id}] FAIL: count(a)={a} decreased from previous checkpoint's {prev_total} "
                  f"-- a word was lost across a checkpoint/recovery")
            ok = False
    ...
    if MAX_CKPT_ID in by_ckpt:
        final = merge(by_ckpt[MAX_CKPT_ID])
